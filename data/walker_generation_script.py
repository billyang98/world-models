...
def _threaded_generation(i):
    ...
    if args.iteration_num is not None:
        tdir = join(
            args.rootdir, "iter_{}".format(args.iteration_num), "thread_{}".format(i)
        )
    makedirs(tdir, exist_ok=True)
    error_run = True
    while error_run:
        cmd = ["xvfb-run", "-s", '"-screen 0 1400x900x24"', "-a"]
        ...
        if args.iteration_num is not None and args.iteration_num > 0:
            ...
            cmd += [
                "python3 test_controller.py --logdir exp_dir --rollouts {} --rollouts_dir {} --iteration_num {}".format(
                    rpt, tdir, args.iteration_num - 1
                )
            ]
        else:
